...
ALTER TABLE public.reports ADD COLUMN resolution_photos TEXT[];
ALTER TABLE public.reports ADD COLUMN estimated_completion TIMESTAMPTZ;
...
CREATE INDEX idx_operator_assignments_report_id ON public.operator_assignments(report_id);
CREATE INDEX idx_operator_assignments_operator_id ON public.operator_assignments(operator_id);
CREATE INDEX idx_operator_assignments_status ON public.operator_assignments(status);
...
CREATE INDEX idx_operator_activity_logs_operator_id ON public.operator_activity_logs(operator_id);
CREATE INDEX idx_operator_activity_logs_report_id ON public.operator_activity_logs(report_id);
CREATE INDEX idx_operator_activity_logs_action ON public.operator_activity_logs(action);
CREATE INDEX idx_operator_activity_logs_created_at ON public.operator_activity_logs(created_at DESC);

-- Enable RLS on new tables
ALTER TABLE public.operator_assignments ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.operator_activity_logs ENABLE ROW LEVEL SECURITY;

-- RLS Policies for operator_assignments
CREATE POLICY "Operators can view their own assignments"
  ON public.operator_assignments FOR SELECT
...
CREATE POLICY "Admins can create assignments"
  ON public.operator_assignments FOR INSERT
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.administrators
      WHERE id = auth.uid()
...
CREATE POLICY "Operators can create their own activity logs"
  ON public.operator_activity_logs FOR INSERT
...
BEGIN
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    INSERT INTO public.operator_activity_logs (operator_id, report_id, action, details)
    VALUES (
...
BEGIN
  ...
  JOIN public.reports r ON r.id = oa.report_id
